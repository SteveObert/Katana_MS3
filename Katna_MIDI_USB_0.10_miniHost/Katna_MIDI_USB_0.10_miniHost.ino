// Working with nano and mini USB host sheild
...
#define TCAADDR 0x70     // Multiplexer address for displays
...
//###########################
//###########################
// Setup the TCA9548A multiplexer for multple displays
...
void setEdit(void) {
  ...
  byte dataTest = 0;
  Serial.println();      Serial.println(F("Waiting..."));
  Serial.println();
  //MS3 katana;
katana.begin();
  ...
  if (effects == 1) {
    if (digitalRead(SW1) == LOW && millis() - lastRead > db) {
      if (effectsState1 == 0) {
        ...
        display.setTextColor(WHITE);
        display.setCursor(10, 0);
        display.println(F("Button 1 Off"));
        display.display();
      }
    }
    if (digitalRead(SW2) == LOW && millis() - lastRead > db) {
      if (effectsState2 == 0) {
        ...
        display.setTextColor(WHITE);
        display.setCursor(10, 0);
        display.println(F("Button 2 Off"));
        display.display();
      }
    }
  }
